function [G, nethandles] = somTrainingPlus(inputData, initnetwork, options)
...
if nargin<3
    ...
    [alphazero, alphadtype, N0, ndtype, maxiter, staticfrontier, steptype, ...
        debugvar, gifolder, gifname] = getOptions(options);
end
...
if debugvar==true
   % figure(99)
    somScatterGraphPlot(inputData(:,1:2), [], OG);
...
for k=1:maxiter
    ...
    thisinput = inputData(ridx,:);
    thisalpha = alphavect(k);
    thisnumneighbors = neighvect(k);
    
    stepOptions.thisalpha = thisalpha;
    stepOptions.numneighbors = thisnumneighbors;
    stepOptions.staticfrontier = staticfrontier;
    stepOptions.steptype = steptype;
    
    [G, nethandles] = somUpdateStep(G, thisinput, stepOptions);
    ...
    if debugvar==true
        ...
        title(strcat('Analysing cell shape:', 32, num2str(k),'/', num2str(maxiter)));
...
if debugvar == true && ~isempty(gifname)
     outputname = strcat(gifolder, gifname);
     imwrite(im,map,outputname,'DelayTime',0.35, 'LoopCount',inf);
end
...
switch lower(decaytype)
    ...
    case 'exp'
        qq = 2/N0;
        neighval = ceil(N0.*(qq + (1-qq).*exp(-kvect.^2./maxiter)));
    otherwise
        neighval = N0.*ones(maxiter,1);
end
end
function [alphazero, alphadtype, N0, ndtype, maxiter, staticfrontier, steptype, ...
...
for ix=1:length(fnames)
    ...
    switch name
        ...
        otherwise
            str = strcat('[somTrainingPLUS].ERROR. Incorrect option', 32,...
